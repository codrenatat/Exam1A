    ...
    for(int i = 0; i < 30; i++){
        for(int j = 0; j < 30; j++){
            ...
            if(matrix_cells <= 540){
                ...
                if(matrix_cells <= 135){ //Sin vacunar y sin cubrebocas.
                    (*(matrix + i) + j)->vaccine = False;
                    (*(matrix + i) + j)->mask = No;
                    (*(matrix + i) + j)->letter = 'S';
                }else if (matrix_cells <= 270) { // Sin vacunar y con cubrebocas
                    (*(matrix + i) + j)->vaccine = False;
                    (*(matrix + i) + j)->mask = Yes;
                    (*(matrix + i) + j)->letter = 's';
                } else if (matrix_cells <= 405) { // Vacunados y sin cubrebocas
                    (*(matrix + i) + j)->vaccine = True;
                    (*(matrix + i) + j)->mask = No;
                    (*(matrix + i) + j)->letter = 'V';
                }else {//Vacunados con cubrebocas
                    (*(matrix + i) + j)->vaccine = True;
                    (*(matrix + i) + j)->mask = Yes;
                    (*(matrix + i) + j)->letter='v';
                }
